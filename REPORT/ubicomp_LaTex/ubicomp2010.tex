...
\begin{abstract}
%background & gab
This paper identifies the possibility of using the BLIP system for smartphone location determining with the purpose of calculating a dynamic playlist on a server, which is responsible for music streaming calculated according to transmitted client playlists via wlan. This idea is inspired by Context DJ and the Follow-Me-Music \ref{key} project proposals by ITU and is a blend of them. %innovation
Although the idea is not novel, the approach of using BLIP system for location determination, wlan for data transmission makes the project non-trivial and adds to its pervasiveness.%contributions
